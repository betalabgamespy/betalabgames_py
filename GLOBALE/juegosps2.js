// juegosps2.js - VERSIÓN CON PAGINACIÓN DINÁMICA
class TiendaPS2 {
    constructor() {
        this.juegos = [];
        this.juegosPorPagina = 12;
        this.paginaActual = 1;
        this.init();
    }
    ...
    async cargarJuegos() {
        try {
            ...
            if (!data.jeugosps2) {
                throw new Error('No se encontró el array "jeugosps2" en el JSON');
            }
            
            this.juegos = data.jeugosps2;
            console.log(`🎮 Se cargaron ${this.juegos.length} juegos PS2`);
            
        } catch (error) {
            console.error('❌ Error cargando juegos PS2:', error);
            this.mostrarError('Error cargando los juegos: ' + error.message);
        }
    }

    mostrarJuegos() {
        const container = document.getElementById('juegos-ps2-container');
        
        if (!container) {
            console.error('❌ No se encontró el contenedor "juegos-ps2-container"');
            return;
        }
        ...
        return `
            <article class="juegos">
                ...
                <img class="juego" src="${juego.imagen}" alt="${juego.Nombre}" 
                     onerror="this.src='https://via.placeholder.com/150x190/3498db/white?text=PS2'">
                <h5 class="precio-act">${juego.precio} Gs</h5>
                <button type="button" class="btn-compra" onclick="tiendaPS2.añadirAlCarrito(${juego.id})">
                    Añadir al carrito
                </button>
            </article>
        `;
    ...
    añadirAlCarrito(id) {
        ...
        if (juego) {
            if (typeof añadirAlCarrito === 'function') {
                ...
                const mensaje = `¡Hola! Quiero comprar PS2: ${juego.Nombre}%0APrecio: ${juego.precio} Gs`;
